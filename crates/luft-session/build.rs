...
fn main() {
    ...
    let shell = target_dir.join("luft-shell");

    let watch_paths = [
        workspace_root.join("crates/kestrel/src"),
        workspace_root.join("crates/kestrel/Cargo.toml"),
        workspace_root.join("crates/luft-shell/src"),
        workspace_root.join("crates/luft-shell/Cargo.toml"),
        workspace_root.join("crates/luft-ipc/src"),
    ];

    if needs_rebuild(&kestrel, &watch_paths) || needs_rebuild(&shell, &watch_paths) {
        println!(
            "cargo:warning=kestrel/luft-shell may be stale; build them with `cargo build -p kestrel -p luft-shell` before running luft-session"
        );
    }
}
