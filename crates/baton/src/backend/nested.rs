...
pub fn run(options: NestedOptions) -> Result<(), NestedError> {
    let mut display: Display<BatonState> = Display::new()?;
    let dh = display.handle();
    let mut state = BatonState::new(&dh, options.config);
    ...
    println!("Baton nested compositor is running");
...
fn bind_socket(socket_name: Option<&str>) -> Result<ListeningSocket, NestedError> {
    match socket_name {
        ...
        None => Ok(ListeningSocket::bind_auto("staccato", 1..33)?),
    }
}
...
fn _keep_seat_type(_: &Seat<BatonState>) {}
