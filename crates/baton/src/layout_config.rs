...
fn configured_workspaces(config: &StaccatoConfig) -> Vec<Workspace> {
    config
        .workspaces
        .entries
        .iter()
...
mod tests {
    use super::*;
    use staccato_config::WorkspaceConfig;
    use staccato_layout::ProfileId;

    #[test]
    fn default_config_materializes_runtime_workspace() {
        let mut config = StaccatoConfig::default();
