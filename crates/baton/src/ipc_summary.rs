...
use staccato_ipc::{ProfileSummary, StatusPayload, WindowSummary, WorkspaceSummary};
use staccato_layout::{ProfileId, WindowState, mode_for_profile};

pub fn status_payload(state: &BatonState) -> StatusPayload {
...
pub fn window_summaries(state: &BatonState) -> Vec<WindowSummary> {
    let active = state
        .windows
...
pub fn workspace_summaries(state: &BatonState) -> Vec<WorkspaceSummary> {
    state
        .layout
        .workspaces()
...
pub fn profile_summaries(state: &BatonState) -> Vec<ProfileSummary> {
...
pub fn known_profiles(state: &BatonState) -> Vec<ProfileId> {
...
fn active_profile(state: &BatonState) -> Option<ProfileId> {
    state
        .layout
        .workspaces()
