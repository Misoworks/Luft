...
use luft_config::{LuftConfig, PinnedAppConfig};

pub fn pin_app(
    config: &mut LuftConfig,
    current: &[PanelApp],
    label: String,
    command: String,
    icon: Option<String>,
...
pub fn unpin_app(config: &mut LuftConfig, current: &[PanelApp], command: &str) -> bool {
    materialize(config, current);
...
pub fn reorder_apps(config: &mut LuftConfig, current: &[PanelApp], commands: Vec<String>) -> bool {
...
fn materialize(config: &mut LuftConfig, current: &[PanelApp]) {
